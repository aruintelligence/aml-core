...
try {
  ...
  const packResult = JSON.parse(run(npmBin, ['pack', '--json'], root))[0];
  ...
  run(npmBin, ['install', '--ignore-scripts', '--save-exact', tarballPath], tempRoot);
  ...
  const doctorOutput = run(npxBin, ['--no-install', 'aml-doctor'], tempRoot);
  const validateOutput = run(npxBin, ['--no-install', 'aml', 'validate', 'sample.aml'], tempRoot);
