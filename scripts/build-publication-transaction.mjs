...
const transaction = {
  ...
  artifact: {
    ...
    provenance_root_sha256: binding.release_provenance_root_sha256 || binding.provenance_root_sha256 || binding.release_provenance?.evidence_root_sha256 || null,
    sha256: binding.artifact?.sha256 || binding.tgz?.sha256 || binding.package?.sha256 || null,
    npm_integrity: binding.artifact?.npm_integrity || binding.tgz?.npm_integrity || binding.package?.npm_integrity || null,
    npm_shasum: binding.artifact?.npm_shasum || binding.tgz?.npm_shasum || binding.package?.npm_shasum || null
  },
...
const rootMaterial = JSON.stringify(transaction);
transaction.transaction_root_sha256 = sha256(rootMaterial);
if (!/^[a-f0-9]{64}$/.test(transaction.transaction_root_sha256)) throw new Error('Invalid transaction root');
